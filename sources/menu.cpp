...
void EApplication::menu(int argc, const char** argv) { 
        ...
        m_desk.add_options()("help", "âûçîâ ñïðàâêè")
            ...
            ("input", boost::program_options::value<std::string>(&input_path)->composing()->default_value("data.txt"), "Ïóòü ê ôàéëó")
            ("threshold", boost::program_options::value<int>(&threshold)->composing()->default_value(1), "ïîðîã");
        try {
        boost::program_options::store(boost::program_options::parse_command_line(argc, argv, m_desk), m_vm);
        }
        catch (std::string ex) {
            std::cout << "Ïîæàëóéñòà, èñïîëüçóéòå --help îïöèþ äëÿ âûâîäà ñïðàâêè"
                << std::endl;
        }
...
int EApplication::exec() {
    if (m_vm.count("help")) {
        ...
        if (ar > 1)
        {
            std::cout <<"Àðãóìåíòîâ "<< ar << std::endl;
